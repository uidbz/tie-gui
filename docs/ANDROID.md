...
./build-android.sh                   # both APKs, android/arm64, with libmpv
...
reproducible) — `third_party/android-libs/` is gitignored and regenerated by
`./vendor-android-libs.sh`.

Build the libraries with the
[mpv-android](https://github.com/mpv-android/mpv-android) build scripts, then
vendor them:

```sh
# in a checkout of mpv-android/buildscripts, with NDK r29:
./buildall.sh --arch arm64 mpv     # builds ffmpeg + libmpv + deps for arm64

# then, from this repo root:
./vendor-android-libs.sh           # copies + strips into third_party/android-libs/
```
...
the runtime linker resolves them from the APK. Override `MPV_PREFIX` /
`ANDROID_NDK_HOME` if your paths differ.
